...
class Backward(nn.Module):
    ...
    def forward(self, tensorInput, tensorFlow):
        ...
        tensorOutput = nn.functional.grid_sample(
            input=tensorInput,
...
class Decoder(nn.Module):
    def __init__(self, intLevel):
        ...
        self.moduleFiv = nn.Sequential(
            nn.Conv2d(
                in_channels=intCurrent + 128 + 128 + 96 + 64, out_channels=32, kernel_size=3, stride=1, padding=1
            ),
            nn.LeakyReLU(inplace=False, negative_slope=0.1),
        )
        ...
        self.moduleSix = nn.Sequential(
            nn.Conv2d(
                in_channels=intCurrent + 128 + 128 + 96 + 64 + 32, out_channels=2, kernel_size=3, stride=1, padding=1
            )
        )
    ...
    def forward(self, tensorFirst, tensorSecond, objectPrevious):
        ...
        if objectPrevious is None:
            ...
            tensorFeat = torch.cat([tensorVolume], dim=1)
            ...
            tensorVolume = self.moduleCorreleaky(
                self.moduleCorrelation(tensorFirst, self.moduleBackward(tensorSecond, tensorFlow * self.dblBackward))
            )
            tensorFeat = torch.cat([tensorVolume, tensorFirst, tensorFlow, tensorFeat], dim=1)

        tensorFeat = torch.cat([self.moduleOne(tensorFeat), tensorFeat], dim=1)
        tensorFeat = torch.cat([self.moduleTwo(tensorFeat), tensorFeat], dim=1)
        tensorFeat = torch.cat([self.moduleThr(tensorFeat), tensorFeat], dim=1)
        tensorFeat = torch.cat([self.moduleFou(tensorFeat), tensorFeat], dim=1)
        tensorFeat = torch.cat([self.moduleFiv(tensorFeat), tensorFeat], dim=1)
